...
3. sorted(str)  如果： sorted(“cba”) 会返回 ["a", "b", "c"]
![](assets/一些命令/file-20260901110733534.png)
**数学运算**:
4. 向下取整：a//b
5. 向上取整：ceil(a // b)
6. 取余：a%b 
7. 指数：a ** b
8. 满足条件的最大整数平方根：isqrt(n) （ isqrt(16) = 4  isqrt(17) = 4 因为：4² = 16 ≤ 17, 5² = 25 > 17)
9. A or B，如果A不为空，返回A；如果A为空，返回B
...
**字典**：
字典里的三个最常用的内置方法用来获取它的核心内容：
- `d.keys()`：获取字典里的所有 **键**
- `d.values()`：获取字典里的所有 **值**
- `d.items()`：获取所有的 **键值对**
1. {}**在 Python 中代表空字典**，关键特点：键值对存储，O(1) 的查找速度
2. **defaultdict()** 通用默认值字典， 如果采用defaultdict()字典里面没有该键，那么会自动提供默认值为0的字典
3. Counter() 专为记数设计 
4. 字典的一些常用使用方法：[763. Partition Labels](../hot%20100/贪心算法/763.%20Partition%20Labels.md) （参考答案第一行）
```python
...
**map()函数**：map前可以用list,max,sum
1. 用于将一个函数应用到可迭代对象（如列表、元组等）的每个元素上 [139. Word Break](../hot%20100/动态规划/139.%20Word%20Break.md) 
```python
numbers = [1, 2, 3, 4, 5]
...
**二维列表**：
1. 二维列表的创建：通常需要for循环来创建，比如[118. Pascal‘s Triangle](../hot%20100/动态规划/118.%20Pascal‘s%20Triangle.md) 
```python
# 创建3行4列的二维数组
