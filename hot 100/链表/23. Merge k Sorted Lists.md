**刷题日期**：2026-2-26
**难度**：Hard

## 题目截图：
---
![](assets/23.%20Merge%20k%20Sorted%20Lists/file-20260226111726218.png)

## 参考答案：
---
方法一（最小堆）：
```python
ListNode.__lt__ = lambda a, b: a.val < b.val  # 让堆可以比较节点大小
  
class Solution:
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        cur = dummy = ListNode()  # 哨兵节点，作为合并后链表头节点的前一个节点
        h = [head for head in lists if head]  # 把所有非空链表的头节点入堆
        heapify(h)  # 堆化
        while h:  # 循环直到堆为空
            node = heappop(h)  # 剩余节点中的最小节点
            if node.next:  # 下一个节点不为空
                heappush(h, node.next)  # 下一个节点有可能是最小节点，入堆
            cur.next = node  # 把 node 添加到新链表的末尾
            cur = cur.next  # 准备合并下一个节点
        return dummy.next  # 哨兵节点的下一个节点就是新链表的头节点
```
- 时间复杂度：O(Llogm)，其中 m 为 lists 的长度，L 为所有链表的长度之和。
- 空间复杂度：O(m)。堆中至多有 m 个元素。

**注意**：
1. 第一行代码：ListNode.__  lt __ 让堆可以比较各节点大小，可以使得heapify的时候让小的值的节点上浮，这是因为ListNode本身没有定义比较方法，直接比较会出现错误
```python
1. # 错误示例
node1 = ListNode(5)
node2 = ListNode(3)
# node1 < node2  # TypeError: '<' not supported between instances of 'ListNode' and 'ListNode'
```
![500](assets/23.%20Merge%20k%20Sorted%20Lists/file-20260226112634842.png)
2. 如果比较的是普通值，不是链表节点的值，那么可以不定义ListNode.__ lt __ ， 直接heapify 

方法二（分治）：
递归
```python
class Solution:
    # 21. 合并两个有序链表
    def mergeTwoLists(self, list1: Optional[ListNode], list2: Optional[ListNode]) -> Optional[ListNode]:
        cur = dummy = ListNode()  # 用哨兵节点简化代码逻辑
        while list1 and list2:
            if list1.val < list2.val:
                cur.next = list1  # 把 list1 加到新链表中
                list1 = list1.next
            else:  # 注：相等的情况加哪个节点都是可以的
                cur.next = list2  # 把 list2 加到新链表中
                list2 = list2.next
            cur = cur.next
        cur.next = list1 if list1 else list2  # 拼接剩余链表
        return dummy.next
  
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        m = len(lists)
        if m == 0:
            return None
        if m == 1:
            return lists[0]  # 无需合并，直接返回
        left = self.mergeKLists(lists[:m // 2])  # 合并左半部分  表示从索引0到 索引m//2 - 1的所有元素 
        right = self.mergeKLists(lists[m // 2:])  # 合并右半部分
        return self.mergeTwoLists(left, right)  # 最后把左半和右半合并
```
时间复杂度：O(Llogm)，其中 m 为 lists 的长度，L 为所有链表的长度之和。每个节点参与链表合并的次数为 O(logm) 次，一共有 L 个节点，所以总的时间复杂度为 O(Llogm)。
空间复杂度：O(logm)。递归深度为 O(logm)，需要 O(logm) 的栈空间。Python 忽略切片产生的额外空间。


![](assets/23.%20Merge%20k%20Sorted%20Lists/file-20260226115558864.png)
