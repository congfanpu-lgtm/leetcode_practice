...
2. ListNode.__ lt __ = lambda a, b: a.val < b.val  # 让堆可以比较节点大小  自定义 链表比较打下的方法，使得堆可以给链表节点的值比较大小

**队列**：
1. deque（双端队列）：![](assets/一些命令/file-20260227111417955.png)

**集合**：set()

**矩阵**：
1. cnt = Counter(c for row in board for c in row)  # 统计矩阵board中每个字符的出现次数 
...
cache装饰器：用于缓存函数结果，避免重复计算，显著提高函数性能。@cache
